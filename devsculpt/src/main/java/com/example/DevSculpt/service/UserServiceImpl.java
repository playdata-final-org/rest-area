...
public class UserServiceImpl implements UserService {
    ...
    @Override
    public UserResponseDTO createDefaultRoleUser(UserRequestDTO requestDTO) {
        // Model Mapper 를 사용해서 DTO -> 엔티티 변환 작업
        ModelMapper mapper = new ModelMapper();
        UserEntity userentity = mapper.map(requestDTO, UserEntity.class);
        // 비밀번호 복호화 작업, 
        userentity.setPassword(passwordEncoder.encode(requestDTO.getPassword())); //TODO-1 done 암호화 2way 인지, ?

        // 회원가입시, 기본 권한은 USER로 준다.
        userentity.setRole("ROLE_USER"); // TODO-1 done 권한을 다르게 주고 싶으면
    ...
    public UserResponseDTO updateUser(Long id, UserResponseDTO responseDTO) {
        ...
        if (existingEntity == null) { // TODO-1 done data가 없을때 null return 만으로 충분할지?
            return null; // Result , Optional<>
        }
        ...
        // 업데이트 날짜, 자동 추가, 
        existingEntity.setRevisionDate(LocalDateTime.now()); // TODO-1 done DB 상에서 on update default time set 을 하는건 어떨지?
    ...
    @Override
    public boolean idcheck(String username) { // TODO-1 done 동사+명사 형태가 기본
